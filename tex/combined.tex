...
\subsubsection{11. nmm-conclusion.ipynb}\label{nmm-conclusion.ipynb}}
...
werden die Spieler äquivalent benannt. * Der beginnende Spieler weiß
wird als \texttt{w} dargestellt, * der gegnerische Spieler als
\texttt{b}.
...
Parameter * \texttt{stash} eine weitere Tupel, bestehend aus
\(\langle w, b \rangle\) und * \texttt{board} eine Tripel, welche die
...
Reihe ist. Dies ist der Fall, g.d.w. * einer der Spieler
\(p \in Players\) weniger als 3 Steine hat (\texttt{hasEnoughStones})
oder * der Spieler \texttt{p} keinen legalen Zug mehr tätigen kann.
...
Gewichtungen, eine für jede Eigenschaft: * \texttt{stones}: Diese
Eigenschaft zählt, wie viele Steine der Spieler auf dem Spielbrett hat.
...
Bei den Spiegelungen wird an vier Achsen gespiegelt: * die
\emph{horizontale} und \emph{vertikale} Achse, sowie * die Diagonale von
...
Funktion in Sekunden; * \texttt{limit} beschreibt die verwendete
maximale Rekursionstiefe; * \texttt{cache\_hit} ist die Anzahl der
Berechnungen, die durch die Transpositionstabelle (\texttt{cache})
eingespart werden konnten; * \texttt{cache\_miss} hingegen ist die
...
In der GUI gibt es drei verschiedene Textarten: - Spielnachricht (wer
spielt gerade oder ob das Spiel beendet ist), - Hinweise (falls zum
Beispiel eine ungültige Aktion ausgeführt worden ist) und -
...
\texttt{movedStone} ist die Koordinate des bewegten Steins des Spielers;
- \texttt{poundedStones} ist eine Menge von Zwei-Tupeln der Form
\texttt{\textless{}op,\ coord\textgreater{}} mit - \texttt{op} ist der
Gegenspieler; - \texttt{coord} ist die Koordinate des geschlagenden
Steines;
...
Verpflichtend: * \texttt{white} ist eine Instanz einer
\texttt{ArtificialIntelligence}, die den weißen Spieler spielen wird; *
\texttt{black} ist eine Instanz einer \texttt{ArtificialIntelligence},
die den schwarzen Spieler spielen wird;

Optional: * \texttt{start\_state} ist der Startzustand, der verwendet
werden soll, standardmäßig \texttt{s0}; * \texttt{start\_player} ist der
Spieler, der das Spiel beginnen soll, standardmäig \texttt{w}; *
\texttt{max\_turns} ist die maximale Anzahl an Zügen, die das Spiel
dauern darf, bevor es in einem Remis endet, standardmäßig \texttt{250};
* \texttt{max\_state\_replayed} ist die maximale Anzahl die ein Zug
nochmal gespielt werden darf, bevor das Spiel in einem Remis endet,
standardmäßig \texttt{5}; * \texttt{max\_states\_without\_mill} ist die
maximale Anzahl von aufeinanderfolgenden Zügen in denen kein Stein
geschlagen wurde, bevor das Spiel in einem Remis endet, standardmäßig
\texttt{30}; * \texttt{name} ein optionaler Name für das Spiel.

Des Weiteren werden zwei Attribute initialisiert, die für den
Spielverlauf nötig sind: * \texttt{log} der Verlauf aller Zustände; *
...
gesetzt werden müssen: * \texttt{winner} \(\in Player \cup \{'\ '\}\),
der Gewinner (oder Remis) des Spieles; * \texttt{log} ist die
chronologische Liste aller gespielen Zustände; * \texttt{reason} eine
Zeichenkette die genauer beschreibt warum das Spiel endete.
...
\texttt{None}, zurückgegeben. Gründe für ein Remis sind: * die maximale
Anzahl an Zügen wurde gespielt - \texttt{max\_turns}; * ein Zug wurde
öfter als maximal erlaubt gespielt - \texttt{max\_state\_replayed}; *
die maximale Anzahl an Zügen ohne eine neue Mühle wurde gespielt -
\texttt{max\_states\_without\_mill}.
...
drei Parameter im Konstruktor: * \texttt{white} ist eine Instanz oder
eine Funktion die eine \texttt{ArtificialIntelligence} produziert, die
den weißen Spieler spielt; * \texttt{black} ist eine Instanz oder eine
Funktion die eine \texttt{ArtificialIntelligence} produziert, die den
schwarzen Spieler spielt; * \texttt{instances} ist die Anzahl der Spiele
die gleichzeitig gestartet werden sollen; * \texttt{seed\_offset} ist
die Zahl, die auf den Seed addiert werden soll.
...
Verpflichtend: * \texttt{participants} ist eine Liste an Teilnehmer
\texttt{ArtificialIntelligence} Instanzen oder Funktionen, die diese
erzeugen;

Optional: * \texttt{instances\_per\_round} ist die Anzahl der Spiele die
...
\texttt{skip} ist die Anzahl der Runden, die übersprungen werden sollen.
Dies ist Hilfreich falls die Berechnungen abbrechen und fortgesetzt
werden sollen; * \texttt{seed\_offset} ist die Zahl, die auf den Seed
...
Parameter: * \texttt{idx} ist die Nummer der aktuellen Runde (wird um
eins erhöht, um eine natürliche Zählung zu verwenden); * \texttt{rnd}
ist die aktuelle Runde; * \texttt{results} ist die Liste der Ergebnisse,
